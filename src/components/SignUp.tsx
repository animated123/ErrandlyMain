...
const SignUp: React.FC<SignUpProps> = ({ onSuccess, onSwitchToLogin, logoUrl }) => {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      // Step 1: Verify Phone if not already verified
      if (!phoneVerified) {
        if (!otpSent) {
      ...
      if (err.code === 'auth/email-already-in-use') {
        ...
        friendlyError = "The verification code you entered is invalid.";
      }
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative group">
            ...
            <input 
              ...
              className="w-full pl-12 pr-6 py-4 bg-muted border-none rounded-[1.25rem] font-black text-foreground text-sm outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all placeholder:text-muted-foreground/70 disabled:opacity-50"
            />
            {phoneVerified && (
          ...
          >
            ...
            <div className="relative group">
              ...
              <input 
                ...
                maxLength={6}
                className="w-full pl-12 pr-6 py-4 bg-white dark:bg-slate-900 border-2 border-indigo-500/20 rounded-[1.25rem] font-black text-foreground text-center text-xl tracking-[0.5em] outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all placeholder:text-muted-foreground/40 placeholder:tracking-normal placeholder:text-sm"
              />
            </div>
          ...
          <AnimatePresence mode="wait">
            {isProcessing ? (
              ...
              <motion.div 
                ...
                className="flex items-center gap-3"
              >
                <span>{otpSent ? 'Verify OTP' : (phoneVerified ? 'Create Account' : 'Verify Phone')}</span>
