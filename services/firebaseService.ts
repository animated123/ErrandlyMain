...
export const firebaseService = {
  ...
  register: async (name: string, email: string, phone: string, pass: string, role?: UserRole): Promise<User> => {
    try {
      ...
      if (!formattedPhone || formattedPhone.length < 5) {
        throw new Error('Please enter a valid phone number (e.g. 0712...)');
      }
  ...
  setupRecaptcha: async (_containerId: string) => {
    console.log('[Auth] Custom SMS flow: reCAPTCHA bypassed');
    return null;
  },
  ...
  sendPhoneVerificationCode: async (phoneNumber: string, _recaptchaVerifier: any) => {
    try {
      ...
      return { phoneNumber: formattedPhone, ...data };
    } catch (error: any) {
  ...
  verifyPhoneCodeAndLogin: async (verifyData: any, code: string) => {
    try {
      ...
      if (data.user) {
        ...
        localStorage.setItem('errand_runner_jwt_token', data.token || `token_${mappedUser.id}`);
        localStorage.setItem('errand_runner_user_profile', JSON.stringify(mappedUser));
        firebaseService._broadcastAuthChange(mappedUser);
        return mappedUser;
      }
      
      throw new Error('Verification successful but no user found. Please register first.');
    } catch (error: any) {
  ...
  sendSmsVerificationCode: async (userId: string, phone: string) => {
    try {
      const formattedPhone = actionService.formatPhoneNumber(phone);
  ...
  verifySmsCode: async (userId: string, phone: string, code: string) => {
    try {
      const formattedPhone = actionService.formatPhoneNumber(phone);
